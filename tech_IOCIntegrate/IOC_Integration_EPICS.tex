...
위와 같이 siteApps 에는 기존에 만들어 놓은 IOC 를 돌릴 수 있는 Application 파일이 있는 폴더들이 존재한다.
...
위와 같은 폴더가(bin,db,dbd 디렉토리) 모두 존재해야 IOC 가 제대로 작동한다.
위의 디렉토리를 생성하기 위해서는 technote_snmp_epics 의 makeBaseApp.pl 디렉토리를
만들어서 make 하는 방법을 참조하자.
...
## 아래는 Application 이 사용할 Library 가 있는 폴더이다 ##
## Snmp 의 경우 siteLibs/lib/linux-x86_64 의 경로에 ( .so 및 .a) 파일이 존재한다.
...
## devIocStat db 들을 아래와 같이 추가하였다. 형식은 devIocStat 에 있는 st.cmd 파일을 참고함. ##
